...
class ICLModel():
    def __init__(self, positive_db: List[dict], negative_db: List[dict]) -> None:
        ...
        # Force SentenceTransformer to initialize on the chosen device
        self.model = SentenceTransformer(
            "all-mpnet-base-v2",
            cache_folder="./model_cache/sentence_transformer/all-mpnet-base-v2",
            device='cpu'
        )
    ...
    def encode_db(self, db, max_bsz=32):
        ...
        with torch.no_grad():
            for idx in range(num_batches):
                ...
                curr_embeddings = self.model.encode(
                    curr_batch,
        ...
        print(f"[ICLModel] Encoded {db_embeddings.size(0)} items on {self.device}")
        return db_embeddings

    def search_examples(self, question, ambig_num, unambig_num):
        # Encode query on the same device and tensor type as the corpus
        with torch.no_grad():
            if isinstance(question, str):
                question_embedding = self.model.encode(
                    [question], convert_to_numpy=False, convert_to_tensor=True, device=self.device
                )
            else:
                question_embedding = self.model.encode(
                    question, convert_to_numpy=False, convert_to_tensor=True, device=self.device
        ...
        # sentence-transformers util accepts torch tensors on the same device
    ...
    def format_icl_prompt(self, question, ambig_num, unambig_num):
        ...
        pos_icl_examples, neg_icl_examples = [], []

        for idx in pos_idxs:
            example = self.positive_db[idx]
            q = example["question"]
            rewrites = self.deduplicate(example["gt_rewrite"])
            all_formated_clarifies = "\n".join(
                clarify_format.format(idx=i + 1, clarification=rw) for i, rw in enumerate(rewrites)
            )
        ...
        for idx in neg_idxs:
            ...
            q = example["question"]
            icl_exp = example_clarify.format(orig_question=q, all_clarifications="No clarification needed.")
